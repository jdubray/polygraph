...
an archaeology tier that replays released version pairs of an open-source
commerce backend and scores against the maintainers' own record. The
pre-registration, the taxonomy, and the null results are committed alongside the
artifacts, and the discipline that matters most is stated here because every
number below depends on it: in the archaeology tier the model of the old version
is authored and committed \emph{before} the new version's diff is read, so a
finding cannot be an artifact of having known the answer.
...
looking at the first capture rather than by theory. Three version changes are
...
classify the first as true-but-intended rather than a true positive: the
maintainers shipped a migration, but it is a constraint widening that addresses
nothing the finding is about, and the pull request that introduced the change
names this inconsistency class in its own description as work still to do.
Reserving the stronger label would have meant stretching a pre-registered rule to
flatter the result.
...
needs no API key; wall-clock times are seconds per change. The real cost is
