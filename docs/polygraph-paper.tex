...
about the status of this claim: it is a literature claim, not an empirical one.
This paper is a design paper. Its mechanism is validated on a committed worked
example rather than on a production fleet, and a real-fleet case study,
underway separately, is the primary future work. We are also careful about the
...
a production fleet. It does not tell us how many distinct states a real fleet
seeds, how the wall-clock time scales, how often a real change passes cleanly
versus fails bounded, or how many landmines the gate catches that a team's
existing process would have missed. Those are exactly the questions a
practitioner will and should ask, and answering them requires a real-fleet case
study, which is underway as a separate effort and is the primary future work of
this line. We flag this rather than paper over it: the design and the mechanism
are validated here; the operational value on real fleets is a claim we have not
yet earned.
...
claim, and it is a literature claim awaiting the empirical one that the planned
fleet study will supply.
...
say so. Every compatibility guarantee is relative to the invariants a human
...
completeness-threshold question Section~\ref{sub:seeded} names. Finally, the author is the
author of the SAM pattern, so the revision study confirms enforcement on the
exact failure classes it was designed against rather than demonstrating that the
enforcement generalizes; an independent replication on a different task is the
right next test, alongside the real-fleet study that is the primary empirical
future work. None of these are reasons to distrust the construction, but all of
them bound what it claims.
...
faces and few name. Versioning stateful code stops being a permanent discipline
