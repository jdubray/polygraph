...
export const stateInvariants = [
  {
    // New in v2: a currency must always be present, because an amount
    // without one cannot be charged.
    name: 'currency-present',
    pred: (s) => typeof s.currency === 'string' && s.currency.length === 3,
  },
  {
  ...
  },
  {
    ...
    pred: (s) => Number.isInteger(s.amountCents) && s.amountCents >= 0,
  },
];
