// v2-dunning — RULES + INTENT: the retry budget drops from 3 to 2, and the
// invariants tighten with it. This is the realistic version of a billing
// policy change, and the live fleet already holds records at depth 2 and 3.
// Expected verdict: INCOMPATIBLE.
//
...
const instance = createInstance({ strict: true, hasAsyncActions: false, instanceName: 'subscription-v2-dunning' });

const MAX_DUNNING = 2; // v2: finance tightened the retry budget from 3 to 2
